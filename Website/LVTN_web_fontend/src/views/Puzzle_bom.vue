      ...
      <div class="museum-chat-header">
        <div class="header-bot-info">
          ...
          <div>
            ...
            <span class="status-dot">Trực tuyến về Bom</span>
          </div>
        </div>
...
const currentArtifactName = ref("Bom");
const currentArtifactId = ref(1);
const MAX_LIVES = 3;
const BASE_SCORE = 10;
...
async function handleCornerClick(corner) {
  ...
  playSynthSound(440, "sine", 0.08);
  try {
    const res = await fetch("http://localhost:8004/random-question-bom");
...
async function initGame() {
  try {
    const res = await fetch("http://localhost:3000/api/puzzle/bom");
    const data = await res.json();
    ...
    const res2 = await fetch("http://localhost:8003/game/decrypt-bom", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: "20260609_021522",
        wm_dir: "D:/LUAN_VAN/MaHoa/runs/Bom/game_images",
      }),
    });
...
async function reveal(index) {
  ...
  try {
    const res = await fetch("http://localhost:3000/api/game/question_bom", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
const chatMessages = ref([             
  {
    sender: "bot",
    text: "Xin chào! Tôi là thuyết minh viên quân đội tại Bảo tàng Quân khu 9. Bạn có thắc mắc gì về hiện vật Bom không?",
    suggestions: ["Lịch sử về Bom", "Chiến công nổi bật", "Thông số kỹ thuật"]
  }
]);
