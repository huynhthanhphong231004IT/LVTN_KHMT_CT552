      ...
      <div class="museum-chat-header">
        <div class="header-bot-info">
          ...
          <div>
            ...
            <span class="status-dot">Trực tuyến về bệ và đạn tên lửa SAM</span>
          </div>
        </div>
...
const currentArtifactName = ref("Bệ và đạn tên lửa SAM");
const currentArtifactId = ref(0);
const MAX_LIVES = 3;
const BASE_SCORE = 10;
...
async function handleCornerClick(corner) {
  ...
  playSynthSound(440, "sine", 0.08);
  try {
    const res = await fetch("http://localhost:8004/random-question-bevadantenlua");
...
async function initGame() {
  try {
    const res = await fetch("http://localhost:3000/api/puzzle/bevadantenlua");
    const data = await res.json();
    ...
    const res2 = await fetch("http://localhost:8003/game/decrypt-bevadantenlua", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: "20260609_025816",
        wm_dir: "D:/LUAN_VAN/MaHoa/runs/bevadantenlua/game_images",
      }),
    });
...
async function reveal(index) {
  ...
  try {
    const res = await fetch("http://localhost:3000/api/game/question_bevadantenlua", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
const chatMessages = ref([             
  {
    sender: "bot",
    text: "Xin chào! Tôi là thuyết minh viên quân đội tại Bảo tàng Quân khu 9. Bạn có thắc mắc gì về hiện vật Bệ và đạn tên lửa SAM không?",
    suggestions: ["Lịch sử bệ và đạn tên lửa SAM", "Chiến công nổi bật", "Thông số kỹ thuật"]
  }
]);
