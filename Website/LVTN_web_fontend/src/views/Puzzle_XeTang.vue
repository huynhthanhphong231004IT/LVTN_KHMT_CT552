    ...
    <div class="museum-chat-box" v-if="isChatOpen">
      <div class="museum-chat-header">
        <div class="header-bot-info">
          ...
          <div>
            ...
            <span class="status-dot">Trực tuyến về Xe Tăng</span>
          </div>
        </div>
      ...
      <div class="museum-chat-footer">
        <input 
          type="text" 
          v-model="chatInput" 
          placeholder="Hỏi về lịch sử, thông số xe tăng..." 
...
const currentArtifactName = ref("Xe tăng");
const currentArtifactId = ref(14);
const MAX_LIVES = 3;
const BASE_SCORE = 10;
...
async function handleCornerClick(corner) {
  ...
  playSynthSound(440, "sine", 0.08);
  try {
    const res = await fetch("http://localhost:8004/random-question-xetang");
...
function loadMapProgress() {
  // 1. Lấy lộ trình động từ localStorage (Nếu không có, dùng mảng rỗng để tránh crash)
  const routeRaw = localStorage.getItem('museum_generated_route');
  const lockedPath = routeRaw ? JSON.parse(routeRaw) : [];

  // 2. Lấy trạng thái game hiện tại
  ...
  if (savedState) {
    ...
    // Nếu trong trạng thái có lưu index, ta dùng luôn index đó
    if (parsed.currentPathIndex !== undefined) {
      currentIdx = parsed.currentPathIndex;
    } 
    // Nếu không có index nhưng có ID hiện vật, ta tự động dò vị trí (index) của ID đó trong lộ trình
    ...
    // Cập nhật các biến hiển thị hiện tại
    ...
    // TRƯỜNG HỢP CHƯA CÓ SAVED_STATE (Lần đầu vào game)
    // Tự động lấy phần tử đầu tiên của lộ trình động làm điểm xuất phát
  ...
  // 3. Tiến hành bóc tách dữ liệu 3 nút dựa trên lộ trình động lấy từ localStorage
  const tempNodes = [];
...
async function initGame() {
  try {
    const res = await fetch("http://localhost:3000/api/puzzle/xetang");
    const data = await res.json();
    ...
    const res2 = await fetch("http://localhost:8003/game/decrypt-xetang", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: "20260608_223506",
        wm_dir: "D:/LUAN_VAN/MaHoa/runs/XeTang/game_images",
      }),
    });
...
async function reveal(index) {
  ...
  try {
    const res = await fetch("http://localhost:3000/api/game/question", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
const chatMessages = ref([             
  {
    sender: "bot",
    text: "Xin chào! Tôi là thuyết minh viên quân đội tại Bảo tàng Quân khu 9. Bạn có thắc mắc gì về hiện vật Xe tăng không?",
    suggestions: ["Lịch sử xe tăng", "Chiến công nổi bật", "Thông số kỹ thuật"]
  }
]);
