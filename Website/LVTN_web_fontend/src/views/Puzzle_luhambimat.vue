      ...
      <div class="museum-chat-header">
        <div class="header-bot-info">
          ...
          <div>
            ...
            <span class="status-dot">Trực tuyến về Lu hầm bí mật</span>
          </div>
        </div>
...
const currentArtifactName = ref("Lu hầm bí mật");
const currentArtifactId = ref(3);
const MAX_LIVES = 3;
const BASE_SCORE = 10;
...
async function handleCornerClick(corner) {
  ...
  playSynthSound(440, "sine", 0.08);
  try {
    const res = await fetch("http://localhost:8004/random-question-luhambimat");
...
async function initGame() {
  try {
    const res = await fetch("http://localhost:3000/api/puzzle/luhambimat");
    const data = await res.json();
    ...
    const res2 = await fetch("http://localhost:8003/game/decrypt-luhambimat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: "20260609_021843",
        wm_dir: "D:/LUAN_VAN/MaHoa/runs/luhambimat/game_images",
      }),
    });
...
async function reveal(index) {
  ...
  try {
    const res = await fetch("http://localhost:3000/api/game/question_luhambimat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
const chatMessages = ref([             
  {
    sender: "bot",
    text: "Xin chào! Tôi là thuyết minh viên quân đội tại Bảo tàng Quân khu 9. Bạn có thắc mắc gì về hiện vật lu hầm bí mật không?",
    suggestions: ["Lịch sử về lu hầm bí mật", "Chiến công nổi bật", "Thông số kỹ thuật"]
  }
]);
