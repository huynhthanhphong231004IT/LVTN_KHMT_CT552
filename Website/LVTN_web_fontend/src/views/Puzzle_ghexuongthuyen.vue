      ...
      <div class="museum-chat-header">
        <div class="header-bot-info">
          ...
          <div>
            ...
            <span class="status-dot">Trực tuyến về Ghe Xuồng Thuyền</span>
          </div>
        </div>
...
const currentArtifactName = ref("Ghe xuồng thuyền");
const currentArtifactId = ref(2);
const MAX_LIVES = 3;
const BASE_SCORE = 10;
...
async function handleCornerClick(corner) {
  ...
  playSynthSound(440, "sine", 0.08);
  try {
    const res = await fetch("http://localhost:8004/random-question-ghexuongthuyen");
...
async function initGame() {
  try {
    const res = await fetch("http://localhost:3000/api/puzzle/ghexuongthuyen");
    const data = await res.json();
    ...
    const res2 = await fetch("http://localhost:8003/game/decrypt-ghexuongthuyen", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: "20260609_021659",
        wm_dir: "D:/LUAN_VAN/MaHoa/runs/ghexuongthuyen/game_images",
      }),
    });
...
async function reveal(index) {
  ...
  try {
    const res = await fetch("http://localhost:3000/api/game/question_ghexuongthuyen", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
const chatMessages = ref([             
  {
    sender: "bot",
    text: "Xin chào! Tôi là thuyết minh viên quân đội tại Bảo tàng Quân khu 9. Bạn có thắc mắc gì về hiện vật ghe xuồng và thuyền không?",
    suggestions: ["Lịch sử về ghe xuồng và thuyền", "Chiến công nổi bật", "Thông số kỹ thuật"]
  }
]);
