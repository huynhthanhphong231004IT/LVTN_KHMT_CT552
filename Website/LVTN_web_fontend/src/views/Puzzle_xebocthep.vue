      ...
      <div class="museum-chat-header">
        <div class="header-bot-info">
          ...
          <div>
            ...
            <span class="status-dot">Trực tuyến về Xe bọc thép</span>
          </div>
        </div>
...
const currentArtifactName = ref("Xe bọc thép");
const currentArtifactId = ref(12);
const MAX_LIVES = 3;
const BASE_SCORE = 10;
...
async function handleCornerClick(corner) {
  ...
  playSynthSound(440, "sine", 0.08);
  try {
    const res = await fetch("http://localhost:8004/random-question-xebocthep");
...
async function initGame() {
  try {
    const res = await fetch("http://localhost:3000/api/puzzle/xebocthep");
    const data = await res.json();
    ...
    const res2 = await fetch("http://localhost:8003/game/decrypt-xebocthep", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: "20260609_022856",
        wm_dir: "D:/LUAN_VAN/MaHoa/runs/xebocthep/game_images",
      }),
    });
...
async function reveal(index) {
  ...
  try {
    const res = await fetch("http://localhost:3000/api/game/question_xebocthep", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
