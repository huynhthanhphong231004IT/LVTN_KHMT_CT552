      ...
      <div class="museum-chat-header">
        <div class="header-bot-info">
          ...
          <div>
            ...
            <span class="status-dot">Trực tuyến về máy bay trực thăng</span>
          </div>
        </div>
...
const currentArtifactName = ref("Máy bay trực thăng");
const currentArtifactId = ref(4);
const MAX_LIVES = 3;
const BASE_SCORE = 10;
...
async function handleCornerClick(corner) {
  ...
  playSynthSound(440, "sine", 0.08);
  try {
    const res = await fetch("http://localhost:8004/random-question-maybaytructhang");
...
async function initGame() {
  try {
    const res = await fetch("http://localhost:3000/api/puzzle/maybaytructhang");
    const data = await res.json();
    ...
    const res2 = await fetch("http://localhost:8003/game/decrypt-maybaytructhang", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: "20260609_021952",
        wm_dir: "D:/LUAN_VAN/MaHoa/runs/maybaytructhang/game_images",
      }),
    });
...
async function reveal(index) {
  ...
  try {
    const res = await fetch("http://localhost:3000/api/game/question_maybaytructhang", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
