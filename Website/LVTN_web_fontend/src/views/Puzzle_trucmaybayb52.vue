      ...
      <div class="museum-chat-header">
        <div class="header-bot-info">
          ...
          <div>
            ...
            <span class="status-dot">Trực tuyến về Trục máy bay B52</span>
          </div>
        </div>
...
const currentArtifactName = ref("Trục máy bay B52");
const currentArtifactId = ref(11);
const MAX_LIVES = 3;
const BASE_SCORE = 10;
...
async function handleCornerClick(corner) {
  ...
  playSynthSound(440, "sine", 0.08);
  try {
    const res = await fetch("http://localhost:8004/random-question-trucmaybayb52");
...
async function initGame() {
  try {
    const res = await fetch("http://localhost:3000/api/puzzle/trucmaybayb52");
    const data = await res.json();
    ...
    const res2 = await fetch("http://localhost:8003/game/decrypt-trucmaybayb52", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: "20260609_022829",
        wm_dir: "D:/LUAN_VAN/MaHoa/runs/trucmaybayb52/game_images",
      }),
    });
...
async function reveal(index) {
  ...
  try {
    const res = await fetch("http://localhost:3000/api/game/question_trucmaybayb52", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
