      ...
      <div class="museum-chat-header">
        <div class="header-bot-info">
          ...
          <div>
            ...
            <span class="status-dot">Trực tuyến về Súng thần công</span>
          </div>
        </div>
...
const currentArtifactName = ref("Súng thần công");
const currentArtifactId = ref(9);
const MAX_LIVES = 3;
const BASE_SCORE = 10;
...
async function handleCornerClick(corner) {
  ...
  playSynthSound(440, "sine", 0.08);
  try {
    const res = await fetch("http://localhost:8004/random-question-sungthancong");
...
async function initGame() {
  try {
    const res = await fetch("http://localhost:3000/api/puzzle/sungthancong");
    const data = await res.json();
    ...
    const res2 = await fetch("http://localhost:8003/game/decrypt-sungthancong", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        run_id: "20260609_022727",
        wm_dir: "D:/LUAN_VAN/MaHoa/runs/sungthancong/game_images",
      }),
    });
...
async function reveal(index) {
  ...
  try {
    const res = await fetch("http://localhost:3000/api/game/question_sungthancong", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
